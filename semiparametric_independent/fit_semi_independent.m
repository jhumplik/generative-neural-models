function out = fit_semi_independent(data, in, options)
%fit_semi_independent Fits semiparametric independent model to data. 
...
%
% Syntax: out = fit_semi_independent(data, in, options)
%
% Inputs:
...
%     .learning_rate_a: Learning rate for a.
...
%
% Required m-files: monotone.m, DmonotoneDt.m, sample_semi_independent.m,
%                   grad_descent.m
...
a0 = in.a;
pars0 = [a0;x0];
learning_rate = [ones(size(a0))*options.learning_rate_a; ...
...
parfor k = 1:n_pools
    ...
    samples_batch(:,:,k) = sample_semi_independent(samples, a0, ...
                                                   V, burn_in);
end
...
samples = sample_semi_independent(samples, a, V, gibbs_steps);
E_samples = samples*a;
...
Da_model = (samples'*dVdE(E_samples)) / length(samples);
...
Da_data = (data'*dVdE(E_data)) / length(data);
% Total gradient.
Dloss_per_batch = [-Da_model + Da_data; -DV_model + DV_data];
